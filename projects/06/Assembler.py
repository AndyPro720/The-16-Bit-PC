...
#running first pass to look for () and stripping all () statements
fpcode=''
count=-1
for fpass in scode.splitlines():
	...
	if id==-1: 
		...
		id1=fpass.find(')')
		sym=fpass[id+1:id1]
		symbols[sym]=str(count+1)

#running second pass to look for var and storing them with register numbers
spcode=''
register=16
	...
	if id==-1: 
		...
		try:
			...
			try:
				...
				sym=symbols[sym]
				spcode=spcode+'@'+sym+'\n'
#Sending assembly code for trasnlation
